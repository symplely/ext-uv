...
#define PHP_UV_SKIP_DTOR(uv)                        \
	do                                                \
	{                                                 \
		GC_FLAGS(&uv->std) |= IS_OBJ_DESTRUCTOR_CALLED; \
	} while (0)
#else
#define PHP_UV_SKIP_DTOR(uv)                          \
	do                                                  \
...
static void php_uv_udp_send_cb(uv_udp_send_t *req, int status)
{
	...
	{ /* send_cb is invoked *before* the handle is marked as inactive - uv_close() will thus *not* increment the refcount and we must then not delete the refcount here */
...
PHP_FUNCTION(uv_stdio_new)
{
	...
	zval *handle;
	zend_long flags = 0;
#if !defined(PHP_WIN32) || defined(HAVE_SOCKET) || PHP_VERSION_ID >= 80000
	php_socket *socket;
#endif
	php_socket_t fd = -1;
	php_stream *stream;
